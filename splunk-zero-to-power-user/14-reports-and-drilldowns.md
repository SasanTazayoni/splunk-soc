...
- **Manage tokens on this dashboard** - instead of navigating away, a click **sets a token** on the current dashboard. You choose **which token** and **what value** it gets - usually the clicked cell, available as **`$click.value$`** (or `$row.<field>$`). Any **other panel** whose search references that token then **re-runs in place** with the new value. It's the same token substitution as the [Tokens in action](#tokens-in-action) example - but triggered by a **click** rather than typed into an **input box**. This is exactly how `$tokens$` pass a value **from panel to panel**: click a value in panel A → it sets the token → panel B (which uses the token) updates.
